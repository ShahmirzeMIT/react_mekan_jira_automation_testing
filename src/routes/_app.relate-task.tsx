...
export default function RelateTaskPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <PageHeader
        title="Relate Task"
        description="Connect a Jira task with the GitHub files that need to change."
      />
      <div className="surface mb-4 flex flex-wrap gap-2 p-3">
        <Select value={taskKey} onValueChange={setTask}>
          <SelectTrigger className="h-9 w-64">
            <SelectValue placeholder="Select Jira task" />
          </SelectTrigger>
          <SelectContent>
            {tasks.map((t) => (
              ...
              return (
                ...
                >
                  <Checkbox
                    id={f.path}
                    checked={checked}
                    onCheckedChange={(v) => (v ? selectFile(f.path) : removeFile(f.path))}
                  />
                  <label htmlFor={f.path} className="flex-1 cursor-pointer font-mono text-xs">
                    {f.path}
                  </label>
        ...
        <aside className="space-y-4">
          <section className="surface p-4">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {ai.selectedFiles.length} files selected
            </p>
            <ul className="mt-3 space-y-1.5">
              {ai.selectedFiles.map((p) => (
                <li key={p} className="flex items-center justify-between gap-2 font-mono text-xs">
                  <span className="truncate">{p}</span>
                  <Button size="sm" variant="ghost" onClick={() => removeFile(p)}>
                    Remove
                  </Button>
                </li>
              ))}
              {!ai.selectedFiles.length && (
                <li className="text-sm text-muted-foreground">No files selected yet.</li>
              )}
            </ul>
            <Button
              ...
              size="sm"
              onClick={send}
              disabled={!ai.selectedFiles.length || !taskKey}
            >
              Send to AI
            </Button>
          </section>
