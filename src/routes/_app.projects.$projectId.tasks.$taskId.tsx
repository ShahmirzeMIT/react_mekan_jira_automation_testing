...
export default function TaskDetailsPage() {
  ...
  if (!task) return <EmptyState title="Task not found" description="This task is not part of the current project." />;
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>Completing this task will synchronize its status with Jira.</AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={complete}>Complete Task</AlertDialogAction></AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </>}
      />
        ...
        <div className="space-y-4">
          <section className="surface p-5">
            ...
            <ul className="mt-2 space-y-1.5 text-sm text-muted-foreground">
              {task.acceptanceCriteria.map((c) => <li key={c}>• {c}</li>)}
            </ul>
          </section>
            ...
            <ul className="mt-3 space-y-3">
              {task.comments.map((c) => (
                <li key={c.id} className="text-sm">
                  <p className="font-medium">{c.author} <span className="font-mono text-[11px] text-muted-foreground">{c.createdAt}</span></p>
                  <p className="text-muted-foreground">{c.body}</p>
                </li>
              ))}
              {!task.comments.length && <li className="text-sm text-muted-foreground">No comments yet.</li>}
            </ul>
            <Textarea className="mt-4" placeholder="Add a comment…" value={comment} onChange={(e) => setComment(e.target.value)} />
            <Button size="sm" className="mt-2" onClick={() => { if (comment.trim()) { toast.success("Comment added."); setComment(""); } }}>Comment</Button>
          </section>
        </div>
        ...
        <aside className="space-y-4">
          <section className="surface p-5 text-sm">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <ConnectionBadge label={task.jiraSynced ? "Jira Synced" : "Jira Not synced"} connected={task.jiraSynced} />
              <ConnectionBadge label={task.githubConnected ? "GitHub Linked" : "GitHub Not linked"} connected={task.githubConnected} />
            </div>
            <dl className="mt-4 space-y-1.5 text-muted-foreground">
              <div className="flex justify-between"><dt>Priority</dt><dd><TaskPriorityBadge priority={task.priority} /></dd></div>
              <div className="flex justify-between"><dt>Assignee</dt><dd>{task.assignee.name}</dd></div>
              <div className="flex justify-between"><dt>Branch</dt><dd className="font-mono text-xs">{task.branch ?? "—"}</dd></div>
            </dl>
          </section>
          ...
          <section className="surface p-5">
            ...
            {task.relatedFiles.length ? (
              <ul className="mt-3 space-y-1.5 font-mono text-xs text-muted-foreground">
                {task.relatedFiles.map((f) => <li key={f}>{f}</li>)}
              </ul>
            ) : (
              <p className="mt-2 text-sm text-muted-foreground">No files are related to this task yet.</p>
            )}
          </section>
        </aside>
      </div>
